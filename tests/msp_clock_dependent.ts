...
import { BN, AnchorError, Program } from '@project-serum/anchor';
...
describe('msp', () => {
  ...
  it('create treasury -> add funds -> create stream (low rate) -> get stream (after 1 sec) -> get stream (after 4 sec)', async () => {
    ...
    // try {
    const streamEvent1 = await mspSetup.getStream(treasurerKeypair, streamKeypair.publicKey);
    ...
    await sleep(3_000);
    const streamEvent2 = await mspSetup.getStream(treasurerKeypair, streamKeypair.publicKey);
