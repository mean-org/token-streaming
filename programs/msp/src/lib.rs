...
pub mod msp {
    ...
    pub fn create_template(
        ctx: Context<CreateTemplateAccounts>,
        start_utc: u64,
        rate_amount_units: u64,
        rate_interval_in_seconds: u64,
        cliff_vest_amount_units: u64,
    ...
    pub fn add_funds<'info>(ctx: Context<AddFundsAccounts>, amount: u64) -> Result<()> {
        let clock = Clock::get()?;
        let now_ts = clock.unix_timestamp as u64;
        ...
        let treasury = &mut ctx.accounts.treasury;
        msg!(
            "clock: {0}, tsy_bal: {1}, tsy_alloc: {2}, tsy_wdths: {3}, add_a: {4}",
            now_ts,
    ...
    pub fn allocate<'info>(ctx: Context<AllocateAccounts>, amount: u64) -> Result<()> {
        let clock = Clock::get()?;
        let now_ts = clock.unix_timestamp as u64;
        let now_slot = clock.slot as u64;
        ...
        let treasury = &mut ctx.accounts.treasury;
        msg!(
            "clock: {0}, tsy_bal: {1}, tsy_alloc: {2}, tsy_wdths: {3}, alloc_a: {4}",
            now_ts,
            treasury.last_known_balance_units,
            treasury.allocation_assigned_units,
            treasury.total_withdrawals_units,
    ...
    /// Close Stream
    pub fn close_stream(ctx: Context<CloseStreamAccounts>) -> Result<()> {
        let clock = Clock::get()?;
        let now_ts = clock.unix_timestamp as u64;
        let now_slot = clock.slot as u64;
    ...
    pub fn close_treasury(ctx: Context<CloseTreasuryAccounts>) -> Result<()> {
        ...
        let treasury = &mut ctx.accounts.treasury;
        msg!(
            "clock: {0}, tsy_bal: {1}, tsy_alloc: {2}, tsy_wdths: {3}",
            now_ts,
            treasury.last_known_balance_units,
            treasury.allocation_assigned_units,
            treasury.total_withdrawals_units
    ...
    pub fn treasury_withdraw(ctx: Context<TreasuryWithdrawAccounts>, amount: u64) -> Result<()> {
        ...
        let treasury = &mut ctx.accounts.treasury;
        msg!(
            "clock: {0}, tsy_bal: {1}, tsy_alloc: {2}, tsy_withds: {3}, withd_a: {4}",
            now_ts,
            treasury.last_known_balance_units,
            treasury.allocation_assigned_units,
            treasury.total_withdrawals_units,
